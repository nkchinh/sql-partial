...
public partial class DuplicateTest
{
    ...
    public void DuplicateExtensions_ShouldDeduplicateProperties_AndIncludeOtherProviders()
    {
        ...
        Assert.Equal("SELECT 'from .ms.sql' FROM System;" + Environment.NewLine, info.SqlServer);

        // Verify GetOther (using .pgsql and .pg.sql collision)
        // Since .pg.sql is longer than .pgsql, it matches first.
        var other = SqlGetOther;
        Assert.Equal("SELECT 'collision' FROM Collision;" + Environment.NewLine, other.PostgreSql);
        ...
        Assert.Equal("SELECT 'from .ms.sql' FROM System;" + Environment.NewLine, info.Get("SqlServer"));
        Assert.Equal("SELECT 'collision' FROM Collision;" + Environment.NewLine, other.Get("PostgreSql"));
    }
}
